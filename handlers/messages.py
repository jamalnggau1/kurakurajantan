...
async def process_lang_invalid(message: Message):
    return await message.reply(
        "<i>Pilih bahasa Anda dari keyboard.\n"
...
async def process_gender_invalid(message: Message):
    ...
    return await message.reply(
        "<i>Pilih jenis kelamin Anda dari keyboard.\n"
...
async def another_message_handler(message: Message):
    ...
    if (
        ...
        and message.text in ["❌ Akhiri pencarian", "❌ End search"]
    ):
        await message.answer(
    ...
    elif message.text in ["✅ Cari Acak", "✅ Search"]:
        await chat.search(message)
    elif message.text in ["👩‍🦱 Cari Perempuan", "👩‍🦱 Chat F"]:
        await chat.search_by_gender(message, sex="F")
    elif message.text in ["👨‍🦱 Cari Laki-laki", "👨‍🦱 Chat M"]:
