...
async def set_user_gender(query: CallbackQuery, state: FSMContext):
    text = ["Jenis kelamin Anda berhasil dipilih.", "Your gender was successfully selected."]
...
async def report_handler(query: CallbackQuery):
    txt = ["Pilih alasan:", "Choose a reason:"]
    language = await db.user_data(query.from_user.id, "language")
...
async def report_reason_handler(query: CallbackQuery, state: FSMContext):
    txt = ["Terima kasih atas tanggapan Anda!", "Thank you for your feedback!"]
    language = await db.user_data(query.from_user.id, "language")
...
async def ban_handler(query: CallbackQuery):
    ...
    await query.message.edit_text(f"[banned](tg://user?id={user_id})_ for {day} days, reason: {texts.reasons[language][x[reason]]}._",
        parse_mode="Markdown",
    )
