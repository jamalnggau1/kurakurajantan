...
async def start_command_handler(message: Message):
    ...
    if row[4] != "Y":
        await message.reply(
            "Pilih bahasa/Choose language ", reply_markup=inline.lang_btn,
        )
        await Form.LANG.set()
    else:
...
async def ban_command_handler(message: Message):
    ...
    if await db.is_banned(user_id) is None:
        ...
        return await message.answer("Anda telah melarang pengguna")
    return await message.answer("Orang ini telah dilarang.")
...
async def unban_command_handler(message):
    ...
    await message.answer("Anda telah membatalkan pemblokiran pengguna")
