...
async def stats():
    ...
    return f"""
Obrolan aktif: {int(len(chatting) / 2)}
Orang yang mencari: {len(serching)}
Orang yang menelusuri menurut jenis kelamin: {len(gender_serching)}
Mencari cowok: {len(male_serching)}
Mencari cewek: {len(female_serching)}
Gadis terdaftar: {len(female)}
Putra Terdaftar: {len(male)}
                            """
